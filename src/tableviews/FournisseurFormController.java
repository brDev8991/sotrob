    ...
    void ajouterFournisseur(ActionEvent event) throws SQLException {
    	if (DBConnection.getConnection() != null) {
			if (!tf1.getText().equals("")) {
				if (DBConnection.checkFournisseurExistance(tf1.getText())) {
					Alert alert = new Alert(AlertType.WARNING);
					alert.setTitle("WARNING");
					alert.setHeaderText("This FOURNISSEUR already exist !");
					alert.show();
				}else {
					DBConnection.ajouterFournisseur(tf1.getText(),
			    			tf2.getText(),
			    			tf3.getText(),
			    			tf4.getText(),
			    			tf5.getText(),
				...
				fournisseurController.refreshTable();				
			}else {
				Alert alert = new Alert(AlertType.WARNING);
				alert.setTitle("WARNING");
				alert.setHeaderText("Code FOURNISSEUR Obligatoire");
				alert.show();
			}
		}	
    }
